...
describe("<FormBuilder />", () => {
  ...
  beforeEach(() => {
    wrapper = render(
      <FormBuilder config={formConfig(defaultState)} onSubmit={spy} />
    );
  });
  ...
  describe("Conditonal node method calls", () => {
    ...
    it("unchecks #1 and #2 when you check #3", async () => {
      const check_1 = wrapper.getByTestId("checkOne");
      const check_2 = wrapper.getByTestId("checkTwo");
      const check_3 = wrapper.getByTestId("checkThree");
